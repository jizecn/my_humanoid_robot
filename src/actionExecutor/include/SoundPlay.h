...
public:
	...
	{
		printf("SOUND PLAY CLASS. %s / %s\n", filepath, filename);
		if (!deviceInit)
		{
			fprintf(stderr, "SDL device was not initialised\n");
			return false;
		}
...
private:
	...
	int Open_Audio()
	{
		...
		int t = SDL_OpenAudio(&fmt, &obtained);
		if ( t < 0 )
		{
	...
	void PlaySound(const char *file)
	{
		...
		//open audio to play
		int ret = Open_Audio();
		printf("Open Audio return value is %d \n", ret);
