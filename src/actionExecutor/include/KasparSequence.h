...
class KasparSequence
{
 public:
  ...
  /*
    ...
    //TODO: Change seqSteps.set() to std::vector compatible
    if (!seqSteps.set(seqStep, index))
    {
    printf("index is out of valid scope");
    return false;
    }
    return true;
    }*/
  ...
  KasparFile::RetCode initFromFile(const char *filepath,const char *filename, const char *posefilepath)
	...
	if (i == 0) {
	  ...
	  if(!zjConvertToInt(tmpBuf, steps))
	    {
	      printf("Error in converting string to int\n");
	  ...
	  Tokenizer token(tmpBuf, ",");
	  if(token.NextToken())
	    {
	      poseName = token.GetToken();
	      //strcpy(poseName, token.GetToken());		 
	    }
	  else
	    {
	      fprintf(stderr, "Sequence (pose name) format incorrect (NULL)\n");
	      return KasparFile::FileFormatError;
	    }
	  
	  if(token.NextToken())
	    {
	      poseFileName = token.GetToken();
	      
	    }
	  else
	    {
	      fprintf(stderr, "Sequence (pose filename) format incorrect (NULL)\n");
	      return KasparFile::FileFormatError;
	    }
	  kp.setServos(this->servos);
	  kp.initFromFile(posefilepath, poseFileName.c_str());
	  
	  if(token.NextToken()) {
	    if(!zjConvertToInt(token.GetToken().c_str(), delay)) {
	      fprintf(stderr, "Sequence Delay Number format error\n");
	      return KasparFile::NumberFormatError;
	    }
	  }
	  else {
	    fprintf(stderr, "Sequence (delay) format incorrect (NULL) %s\n", tmpTok);
	    return KasparFile::FileFormatError;
	  }
	...
	i++;
      ...
      //{
      if (i != steps + 2) {
	fprintf(stderr, "Sequence number does not match with line numbers (%d %d)\n", i, steps);
	return KasparFile::FileFormatError;
      }
      //}
      //else
      //{
      //	if (i != steps + 2) {
      //		fprintf(stderr, "Sequence number does not match with line numbers\n");
      //		return KasparFile::FileFormatError;
      //	}
      //}
 ...
 private:
  ...
  std::vector<KasparSequenceStep> seqSteps;
