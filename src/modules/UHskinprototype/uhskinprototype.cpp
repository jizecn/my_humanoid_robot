...
bool UHSkinPrototype::open(yarp::os::Searchable& config)
{
    bool correct=true;
#if SKIN_DEBUG
...
int UHSkinPrototype::read(yarp::sig::Vector &out) 
{
    mutex.wait();
    out=data;
    mutex.post();
...
int UHSkinPrototype::getState(int ch)
{
    return yarp::dev::IAnalogSensor::AS_OK;;
}

int UHSkinPrototype::getChannels()
{
    return sensorsNum;
}

bool UHSkinPrototype::calibrate(int ch, double v)
{
    return true;
}


bool UHSkinPrototype::threadInit()
{
#if SKIN_DEBUG
...
{	
    ...
    for (unsigned int i=0; i<canMessages; i++) {
        ...
        for (int i = 0; i < numOfCards; i ++)  {
            if(id == cards[i].getCardId()) {
                int index=sensorId*12;
                cards[i].setData(msg.getData(), type, sensorId);
            }
        }
    }
...
void UHSkinPrototype::threadRelease()
{
#if SKIN_DEBUG
...
void UHSkinPrototype::initCards(Bottle ids)
{
    this->numOfCards = ids.size();
	int tempid = 0;
	for (int i = 0; i < this->numOfCards; i ++)  {
		tempid = ids.get(i).asInt();
		tempid=0x300 | (tempid << 4);
		//cards[i] = Card(tempid);
        cards.push_back(Card(tempid));
    }
}
