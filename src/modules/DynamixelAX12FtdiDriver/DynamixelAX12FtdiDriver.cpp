...
/*
bool DynamixelAX12FtdiDriver::open_old(yarp::os::Searchable& config) {
    ACE_TRACE("DynamixelAx12Driver::initialize");
    ACE_OS::printf("Opening DynamixelAx12 Device\n");
    ...
    // open and reset everything in case we messed everything up with a control-c..
    if ((retCode = ftdi_usb_open(&ftdic, 0x0403, 0x6001)) < 0) {
        ACE_OS::fprintf(stderr, "unable to open ftdi device: %d (%s)\n", retCode, ftdi_get_error_string(&ftdic));
        return EXIT_FAILURE;
    }
    deviceOpen = true; // Only set to be able to do close()
    ...
    if ((retCode = ftdi_set_baudrate(&ftdic, 57600)) != 0)
        ACE_OS::fprintf(stderr, "Error setting baudrate, ret=%d\n", retCode);
    if (ftdi_set_line_property(&ftdic, BITS_8, STOP_BIT_1, NONE) == -1)
        ACE_OS::fprintf(stderr, "Error setting connection properties");
    if (ftdi_setflowctrl(&ftdic, SIO_DISABLE_FLOW_CTRL) == -1)
...
int DynamixelAX12FtdiDriver::sendCommand(unsigned char id, unsigned char inst[], int size, unsigned char ret[], int &retSize) {
   ...
   // #endif
	...
	retCode = ftdi_write_data(&ftdic, command, 5+size);
...
bool DynamixelAX12FtdiDriver::setRefTorque(int,double)
{
	return NOT_YET_IMPLEMENTED("setRefTorque");
}

bool DynamixelAX12FtdiDriver::setRefTorques(const double *)
{
...
{
	return NOT_YET_IMPLEMENTED("sgetTorqueRange");
}
