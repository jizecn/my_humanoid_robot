...
public:
			...
			if (type=="triangle" || type=="fingertip") {
				...
				if (id>=0 && id<16) {
					if (sensor[id]) {
						...
						sensor[id] = new TouchSensor(gain);
	...
	*/
	TactileClassification::TouchEvent processData()
	{
		...
		for(int k = 0; k < sensorsNum; k++)
		{
			...
			sensor[indexArray[k]]->getFilteredData(sensorsData, k);
		}
		...
		//TactileClassification::TouchEvent te = TactileClassification::simpleThreshold(sensorsData, 12*sensorsNum, 37, 70, this->baselines);
		
		TactileClassification::TouchEvent te = TactileClassification::simpleClassify(sensorsData, 12*sensorsNum, 90, 170);
	...
	bool initLogFile(Searchable &config)
	{
		...
		if(ifLog)
		{
			...
			else
			{
				...
				this->logText("partname", this->robotPart.c_str());
