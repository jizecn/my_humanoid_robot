...
void MainFrame::initLayout(QString configfile) {
    ...
    // reset button
    QPushButton *resetBut = new QPushButton(tr("Adjust Sensors"));
    resetBut->setFont(QFont("Times", 14, QFont::Bold));
    ...
    // new added
	connect(resetBut, SIGNAL(clicked()), this, SLOT(resetSensors()));
...
void MainFrame::resetSensors()
{
	printf("Reset sensors -- resetting baselines\n");
	this->bManager->manualSetDrifts();
}
